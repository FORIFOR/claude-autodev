#!/usr/bin/env python3
"""
Slack Webhook Server for bidirectional communication
SlackからのメッセージやSlashコマンドを受信してプロジェクトを制御
"""
...
def send_slack_response(channel, text, thread_ts=None):
    """Slackにメッセージを送信"""
    if SLACK_BOT_TOKEN:
        # Bot Token使用
        ...
        print("❌ Slack Bot Token or Webhook URL not configured")
        return False
...
def process_claude_command(command_text, channel, thread_ts=None):
    ...
    if text.startswith("claude new "):
        # 新規プロジェクト作成: claude new project-name description
        ...
        # 仕様変更: claude modify project-name changes
...
`claude modify todo-app ダークモード機能を追加`"""
    ...
    # 返信送信
    send_slack_response(channel, reply_msg, thread_ts)
    ...
    try:
        ...
        if event.get('type') == 'message' and not event.get('bot_id'):
            ...
            if text.startswith('claude '):
                print(f"💬 Processing Claude command: {text}")
                
                # メッセージログ記録
                log_entry = f"{datetime.now().isoformat()}: Slack command from {channel}: {text}\n"
                os.makedirs(LOGS_DIR, exist_ok=True)
                with open(os.path.join(LOGS_DIR, "slack_messages.log"), "a", encoding="utf-8") as f:
                    f.write(log_entry)
                
                # コマンド処理
                process_claude_command(text, channel, thread_ts)
...
def slack_slash_commands():
    ...
    if command == '/claude':
        ...
        # ログ記録
        log_entry = f"{datetime.now().isoformat()}: Slack slash command {command} {text}\n"
        os.makedirs(LOGS_DIR, exist_ok=True)
        with open(os.path.join(LOGS_DIR, "slack_messages.log"), "a", encoding="utf-8") as f:
            f.write(log_entry)
...
if __name__ == '__main__':
    print("🤖 Starting Slack Webhook Server for Claude Autodev...")
    print(f"📁 Base directory: {BASE_DIR}")
    print(f"🔗 Events endpoint: http://localhost:5002/slack/events")
    print(f"⚡ Commands endpoint: http://localhost:5002/slack/commands")
